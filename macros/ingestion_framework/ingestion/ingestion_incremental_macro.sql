{% macro ingestion_incremental_macro() %}
    {{ log("INGESTION INCREMENTAL FILES CHECKING", info=True) }}
    ...
    {% if pattern %}
        ...
        {{ log("There Are '" ~ files_list | length ~ "' Incremental Files Retrieved for Ingestion", info=True) }}

        {{ return(files_list) }}

    {% else %}
        {{ log("The 'pattern' Config Hasn't Been Found in Model's YAML, So No Incremental Files are Retrieved", info=True) }}
        {{ return([]) }}
